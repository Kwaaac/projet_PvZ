...
public class PrincipalMenuController {
	...
	public static ArrayList<Plant> startGame(ApplicationContext context, SimpleGameData noImportantData) {
		...
		SimpleGameData dataSelect = new SimpleGameData(SimpleGameData.getSelectionCaseNumber(), 1); // plant selected
																									// bord
		...
		/**
		 * - first you choose to play a new game or reload a save - if new game, you
		 * choose the map - then you select the plants you want ot play
		 */
		while (true) {
			switch (choice) {
			case "menu":
			...
			case "plantSelection":
				view.drawPlantSelection(context, view, viewContent, plantSelectionView, dataBord, dataSelect, width,
						height);
				break;
			}
